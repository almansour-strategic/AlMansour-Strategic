...
st.markdown("""
    ...
    * { font-family: 'Cairo', sans-serif !important; text-align: right; }
    ...
    .whatsapp-float { background: #25d366; color: white !important; padding: 12px 25px; border-radius: 50px; text-decoration: none; font-weight: 700; display: inline-flex; align-items: center; gap: 8px; margin-top: 20px; }
    </style>
...
GLOBAL_CONFIG = {
    "📑 تقرير الإنجاز الدوري | Progress Report": {
        ...
        "hints": ["نسبة الإنجاز الفعلي مقابل المخطط...", "الأسباب الكامنة وراء أي تأخير مرصود...", "المخاطر التي هددت سير العمل...", "الإجراءات العاجلة المنفذة..."]
    },
...
rtype = st.selectbox("حدد التخصص لضبط الأسئلة والمنهجية تلقائياً:", list(GLOBAL_CONFIG.keys()))
cfg = GLOBAL_CONFIG[rtype]

st.markdown('<p class="section-title">البيانات التعريفية للمشروع</p>', unsafe_allow_html=True)
c1, c2 = st.columns(2)
with c1:
    st.text_input("اسم المشروع", placeholder="Project Name")
    st.text_input("الجهة المانحة", placeholder="Donor Agency")
with c2:
    st.text_input("الجهة المنفذة", placeholder="Implementing Agency")
    st.text_input("مكان التنفيذ", placeholder="Location")
...
for i in range(4):
    label = cfg["fields"][i]
    hint = cfg["hints"][i]
    key = f"q{i+1}_{rtype}" # مفتاح فريد لكل نوع تقرير لمنع تداخل البيانات
    ...
    with col_t:
        val = st.text_area("", key=key, height=110, label_visibility="collapsed")
    with col_b:
        ...
        if st.button("✨ تحسين", key=f"btn_{key}"):
            if val:
                st.info(f"الصياغة المقترحة: {val} (تمت معالجة النص استراتيجياً)")
            else:
...
with cg:
    ...
    if st.button("🚀 توليد التقرير الماسي"): st.success("تم التوليد بنجاح!")
    st.markdown('</div>', unsafe_allow_html=True)
with ce:
